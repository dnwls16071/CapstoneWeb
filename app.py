...
def index():
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    cur.execute("SELECT * FROM events WHERE user_id = %s ORDER BY id", (current_user.id,))
...
def upload_csv():
    if request.method == 'POST':
        ...
        if csv_file.filename.endswith('.csv'):
            # Read the CSV data & encoding setting
            csv_data = pd.read_csv(csv_file, encoding='UTF8')
        else:
            return """
            <script>
        ...
        for index, row in csv_data.iterrows():
            ...
            cur.execute("INSERT INTO events (title, start_event, end_event, user_id) VALUES (%s, %s, %s, %s)",[row['title'], start_event, end_event, current_user.id])
...
def insert():
    ...
    if request.method == 'POST':
        ...
        cur.execute("INSERT INTO events (title,start_event,end_event,user_id) VALUES (%s,%s,%s,%s)", [title,start,end,current_user.id])
        conn.commit()
        cur.close()
        msg = 'success'
    return jsonify(msg)
...
def update():
    ...
    if request.method == "POST":
        ...
        id = request.form['id']
        cur.execute("UPDATE events SET title = %s, start_event = %s, end_event = %s WHERE id = %s AND user_id = %s", [title, start, end, id, current_user.id])
        conn.commit()
        cur.close()
        msg = 'success'
    return jsonify(msg)
...
def ajax_delete():
    ...
    if request.method == "POST":
        id = request.form['id']
        cur.execute("DELETE FROM events WHERE id = %s AND user_id = %s", (id, current_user.id))
        conn.commit()
        cur.close()
        msg = "success"
    return jsonify(msg)
...
def delete_all():
        ...
        cur.execute("DELETE FROM events WHERE user_id=%s", [current_user.id])
